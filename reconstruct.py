...
def dps(
    net, latents, 
    A, y, likelihood_step_size=1.0, second_order=False,
...
def main(network_pkl, outdir, subdirs, seeds, class_idx, max_batch_size, device=torch.device('cuda'), **sampler_kwargs):
    ...
    for batch_seeds in tqdm.tqdm(rank_batches, unit='batch', disable=(dist.get_rank() != 0)):
        ...
        class_labels = None
        if net.label_dim:
            class_labels = torch.zeros((net.label_dim, net.label_dim), device=device)[rnd.randint(net.label_dim, size=[batch_size], device=device)]
